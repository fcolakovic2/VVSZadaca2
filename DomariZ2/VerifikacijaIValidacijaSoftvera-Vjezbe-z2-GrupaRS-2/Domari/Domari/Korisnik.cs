...
namespace Domari
{
    public abstract class Korisnik
    {
        ...
        public void PromjenaPassworda(string verifikacija, string novi)
        {
            ...
            string verifikacijaHash = verifikacija.GetHashCode().ToString();
            if (verifikacijaHash == password)
            {
                password = novi.GetHashCode().ToString();
            }
            else
            {
                throw new InvalidOperationException("Netacna verifikacija passworda");
            }
